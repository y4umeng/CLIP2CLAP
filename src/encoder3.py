...
class AttentionAligner3(nn.Module):
    def __init__(self, d_model=360, block_size=512):
        super().__init__()

        self.ff1 = nn.Linear(1, d_model)
        self.ff2 = nn.Linear(d_model, 1)
        self.pos_emb = nn.Embedding(block_size, d_model)
        self.encoder_layer = nn.TransformerEncoderLayer(d_model, nhead=12, dim_feedforward=96)
