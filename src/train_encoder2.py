...
from encoder2 import AttentionAligner2
...
def train():
    ...
    d_model = 96
    n_head = 8 # This implied that each head has a dimension for the key, query, and values of d_model / 6.
    n_layer = 8 # This implies we have 6 turns to mix the embeddigs; this is "Nx" in the paper
    # ------------

    train_dl = DataLoader(train_dataset, batch_size, shuffle=True)
    test_dl = DataLoader(test_dataset, batch_size, shuffle=True)

    model = AttentionAligner2(d_model, block_size)
    with torch.no_grad():
